...
import { kTextureFormatInfo as kTextureFormatInfo2 } from './format_info.js';
...
export const info2asinfo1 = Object.fromEntries(
  Object.entries<any>(kTextureFormatInfo2).map(([format, info]) => [
    format,
    {
      renderable: !!info.render,
      multisample: !!info.render?.multisample,
      resolve: !!info.render?.resolve,
      color: !!info.color,
      depth: !!info.depth,
      stencil: !!info.stencil,
      storage: !!info.color?.storage,
      copySrc: !!(info.color ?? info.depth ?? info.stencil)?.copySrc,
      copyDst: !!(info.color ?? info.depth ?? info.stencil)?.copyDst,
      sampleType: (info.color ?? info.depth ?? info.stencil)?.type,
      bytesPerBlock: (info.color ?? info.depth ?? info.stencil)?.bytes,
      blockWidth: info.block.width,
      blockHeight: info.block.height,
      renderTargetPixelByteCost: info.render?.byteCost,
      renderTargetComponentAlignment: info.render?.alignment,
      feature: info.feature,
      baseFormat: info.baseFormat,
    },
  ])
);
// Diff the following with a JSON diff tool
console.log('info1:');
console.log(JSON.stringify(kTextureFormatInfo));
console.log('info2:');
console.log(JSON.stringify(info2asinfo1));
